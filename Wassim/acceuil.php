<?php
session_start(); // démarrer la session 
// Récupérer l'ID de l'utilisateur à partir de la session
$userId = $_SESSION['user_id'];

// Vérifier le rôle de l'utilisateur à partir de la session
$userRole = $_SESSION['user_role']; // Assurez-vous que vous stockez le rôle de l'utilisateur dans la session lors de la connexion

?>



<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../btsp/css/bootstrap.css">
    <link rel="stylesheet" href="../btsp/css/bootstrap.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css">
    <link href="https://fonts.googleapis.com/css?family=Open+Sans:900&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="style.css">
    <title>Document</title>
</head>
<body>
    <div class="container">
        <nav class="navbar navbar-expand-lg navbar-light ">
            <div class="container-fluid">
              <!-- Logo -->
              <a class="navbar-brand" href="acceuil.php" style="color: black; font-size: larger; font-weight:900;">JobApply</a>

          
              <!-- Toggler button for mobile view -->
              <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNavDropdown" aria-controls="navbarNavDropdown" aria-expanded="false" aria-label="Toggle navigation">
                <span class="navbar-toggler-icon"></span>
              </button>
          
              <!-- Navbar links and buttons -->
              <div class="collapse navbar-collapse" id="navbarNavDropdown">
                <ul class="navbar-nav ms-auto mb-2 mb-lg-0 listt">
                  <!-- Navigation Links -->
                  <li class="nav-item">
                    <a class="nav-link " aria-current="page" href="acceuil.php" style="color: #6c63ff;">Home</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link" id="homelink" href="#">About</a>
                  </li>
                  <li class="nav-item">
                    <a class="nav-link" href="#">Offers</a>
                  </li>
                  <!-- la page des candidats reste confidentiel seul les recruteurs peuvent la voir-->
                  <?php if($userRole == "recruteur") {?>
                  <li class="nav-item">
                  <a class="nav-link" href="../jihane/recruteurHome.html">Candidate</a>
                  </li> 
                  <?php }?>
                  <li class="nav-item">
                    <a class="nav-link" id="contactlink" href="contact.php" style="margin-right: 10px;">Contact us</a>
                  </li>
                </ul>
                <!-- Login and Sign Up buttons for mobile view -->
                <?php if(! isset($_SESSION["userId"])){ ?>
                <ul class="navbar-nav">
                  <li class="nav-item">
                    <a href="/login" class="btn btn-outline-primary me-2" type="button" style="padding: 8px 20px; background-color: #6c63ff;border: none; color: white;">Login</a>
                  </li>
                  <li class="nav-item">
                    <a href="/signup" class="btn btn-primary" type="button" style="padding: 8px 20px;background-color: #ff6347;border: none;">Sign Up</a>
                  </li>
                  <?php }else { ?> 
                  <li>
                  <span
                class="nav-link badge d-flex align-items-center p-1 pe-2 text-secondary-emphasis bg-badge border rounded-pill">
                <img class="nav-link profile rounded-circle me-1" width="24" height="24" src="../media/logo.jpeg"
                    alt="profile" />
                <a class="nav-link" href="../jihane/profilCandidat.html">Username</a>
            </span>
                  </li>
                  <?php }?>
                </ul>
              </div>
            </div>
          </nav>
          
    </div>


    <div class="one-8pn d-wb4 item-zgy ftco-vkj fa-1kp ftco-vsz style-q3RvQ" id="style-q3RvQ">
        <div class="row">
        <div class="col-dm-6">
        <div class="text-4to mt-qte">
          <p class="mb-bz3 mt-qte pt-bg6">
            We have 
            <span class="num-9oo">
              200,000
            </span>
            great job offers you deserve!
          </p>
          <h1 class="mb-lad">
            Largets Job Site In The World
          </h1>
          <div class="search-xti">
            <div class="row-7gg">
              <div class="col-oek">
                <div class="nav-212 nav-vmr text-ea5">
                  <a class="nav-link-hx9 act-pww mr-md-2ml" id="searchform1" href="#v-pills-1">
                    Find a Job
                  </a>
                  <a class="nav-link-hx9" id="searchform2" href="#v-pills-2">
                    Find a Candidate
                  </a>
                </div>
              </div>
              <div class="col-oek">
                <div class="content-12q p-2in">
                  <div class="tab-ggw fa-nys sho-7df act-pww">
                    <form class="search-ikh ">
                      <div class="row-7gg no-j87">
                        <div class="col-ifw mr-md-ajo">
                          <div class="form-group-lv4">
                            <div class="field-tsm">
                              <div class="icon-e59">
                                <img src="../media/briefcase.png" style="width: 15px;" alt="">
                              </div>
                              <input type="text" class="form-control-mpb" placeholder="eg. Garphic. Web Developer">
                            </div>
                          </div>
                        </div>
                        <div class="col-ifw mr-md-ajo">
                          <div class="form-group-lv4">
                            <div class="field-tsm">
                              <div class="select-j91">
                                <div class="icon-e59">
                                  <img src="../media/arrow-down-sign-to-navigate.png" style="width: 15px;" alt="">
                                  
                                </div>
                                <select name="" id="mroyy" class="form-control-mpb">
                                  <option value="">
                                    Category
                                  </option>
                                  <option value="">
                                    Full Time
                                  </option>
                                  <option value="">
                                    Part Time
                                  </option>
                                  <option value="">
                                    Freelance
                                  </option>
                                  <option value="">
                                    Internship
                                  </option>
                                  <option value="">
                                    Temporary
                                  </option>
                                </select>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div class="col-ifw mr-md-ajo">
                          <div class="form-group-lv4">
                            <div class="field-tsm">
                              <div class="icon-e59">
                                <img src="../media/pin.png" style="width: 15px;" alt="">
                                
                              </div>
                              <input type="text" class="form-control-mpb" placeholder="Location">
                            </div>
                          </div>
                        </div>
                        <div class="col-ifw">
                          <div class="form-group-lv4">
                            <div class="field-tsm">
                              <button type="submit" class="form-control-mpb btn-i1x btn-secondary-yzs">
                                Search
                              </button>
                            </div>
                          </div>
                        </div>
                      </div>
                    </form>
                  </div>
                  <div class="tab-ggw fa-nys">
                    <form class="search-ikh">
                      <div class="row-7gg">
                        <div class="col-ifw">
                          <div class="form-group-lv4">
                            <div class="field-tsm">
                              <div class="icon-e59">
                                <span class="icon-bo5">
                                </span>
                              </div>
                              <input type="text" class="form-control-mpb" placeholder="eg. Adam Scott">
                            </div>
                          </div>
                        </div>
                        <div class="col-ifw">
                          <div class="form-group-lv4">
                            <div class="field-tsm">
                              <div class="select-j91">
                                <div class="icon-e59">
                                  <span class="row-zko">
                                  </span>
                                </div>
                                <select name="" id="mroyy" class="form-control-mpb">
                                  <option value="">
                                    Category
                                  </option>
                                  <option value="">
                                    Full Time
                                  </option>
                                  <option value="">
                                    Part Time
                                  </option>
                                  <option value="">
                                    Freelance
                                  </option>
                                  <option value="">
                                    Internship
                                  </option>
                                  <option value="">
                                    Temporary
                                  </option>
                                </select>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div class="col-ifw">
                          <div class="form-group-lv4">
                            <div class="field-tsm">
                              <div class="icon-e59">
                                <span class="icon-tpd">
                                </span>
                              </div>
                              <input type="text" class="form-control-mpb" placeholder="Location">
                            </div>
                          </div>
                        </div>
                        <div class="col-ifw">
                          <div class="form-group-lv4">
                            <div class="field-tsm">
                              <button type="submit" class="form-control-mpb btn-i1x btn-secondary-yzs">
                                Search
                              </button>
                            </div>
                          </div>
                        </div>
                      </div>
                    </form>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        </div>
        <div class="col-md-6">
            <div class="image-container" id="imageBackground">
                <img src="../media/undraw_work_time_lhoj.png" alt="Background Image" class="background-image">
                
              </div>
        </div>
        </div>
      </div>

      <div class="fluid bg-dark wassim" style="margin-top: 710px; background-color: #6c63ff !important;">
        <div class="container" style="padding: 112px 0px;">
        <div class="row">
            <div class="col-md-3">
                <div class="icon" style="margin: 10px;">
                  <img src="../media/resume.png" alt="" style="width: 60px;">
                  
                </div>
                <div class="media-main">
                  <h3 class="heading mb-3 text text-white" style="font-size: 18px; font-weight: bold;">
                    Search Million of jobs
                  </h3>
                  <p style="color: rgb(230, 230, 230);opacity: 0.8;">
                    Embark on your next career move by exploring a vast selection of job openings across various industries. Find roles that align with your skills and passions.
                  </p>
                </div>
            </div>
            <div class="col-md-3">
              <div class="icon" style="margin: 10px;">
                <img src="../media/collaboration.png" alt="" style="width: 60px;">
                
              </div>
              <div class="media-main">
                <h3 class="heading mb-3 text text-white" style="font-size: 18px;font-weight: bold;">
                  Easy To Manage Jobs
                </h3>
                <p style="color: rgb(230, 230, 230);opacity: 0.8;">
                  Simplify the hiring process with our intuitive job management tools. Post vacancies, track applicants, and organize your recruitment efforts all in one place.
                </p>
              </div>
          </div>
          <div class="col-md-3">
            <div class="icon" style="margin: 10px;">
              <img src="../media/team-support.png" alt="" style="width: 60px;">
              
            </div>
            <div class="media-main">
              <h3 class="heading mb-3 text text-white" style="font-size: 18px;font-weight: bold;">
                Top Carrers
              </h3>
              <p style="color:rgb(230, 230, 230);opacity: 0.8;">
                Elevate your professional journey with opportunities that propel you towards growth. Discover positions that offer the challenge and advancement you seek.
              </p>
            </div>
        </div>
        <div class="col-md-3">
          <div class="icon" style="margin: 10px;">
            <img src="../media/employee.png" alt="" style="width: 60px;">
            
          </div>
          <div class="media-main">
            <h3 class="heading mb-3 text text-white " style="font-size: 18px;font-weight: bold;">
              Search Expert Candidates
            </h3>
            <p style="color: rgb(230, 230, 230);opacity: 0.8;">
              Source the perfect candidate for your team from our pool of qualified professionals. Connect with experts who bring the right blend of experience and innovation to your business.
            </p>
          </div>
      </div>
        </div>
        </div>
      </div>

      

        <section class="ftco-section" style="padding-top: 200px; padding-bottom: 150px;">
        <div class="container">
        <div class="row justify-content-center mb-5 pb-3">
        <div class="col-md-7 heading-section text-center ftco-animate">
        <span class="subheading" style="opacity: 0.5; font-size: 18px;">Job Categories</span>
        <h2 class="mb-4" style="font-weight: 900; color:black ;
        margin-bottom: 20px;">Top Categories</h2>
        </div>
        </div>
        <div class="row">
        <div class="col-md-3 ftco-animate">
        <ul class="category wawa">
        <li><a href="../Hassan/index.html">Web Development <br><span class="number">354</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        <li><a href="../Hassan/index.html">Graphic Designer <br><span class="number">143</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        <li><a href="../Hassan/index.html">Multimedia <br><span class="number">100</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        <li><a href="../Hassan/index.html">Advertising <br><span class="number">90</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        </ul>
        </div>
        <div class="col-md-3 ftco-animate">
        <ul class="category wawa">
        <li><a href="../Hassan/index.html">Education &amp; Training <br><span class="number">100</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        <li><a href="../Hassan/index.html">English <br><span class="number">200</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        <li><a href="../Hassan/index.html">Social Media <br><span class="number">300</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        <li><a href="../Hassan/index.html">Writing <br><span class="number">150</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        </ul>
        </div>
        <div class="col-md-3 ftco-animate">
        <ul class="category wawa">
        <li><a href="../Hassan/index.html">PHP Programming <br><span class="number">400</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        <li><a href="../Hassan/index.html">Project Management <br><span class="number">100</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        <li><a href="../Hassan/index.html">Finance Management <br><span class="number">222</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        <li><a href="../Hassan/index.html">Office &amp; Admin <br><span class="number">123</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        </ul>
        </div>
        <div class="col-md-3 ftco-animate">
        <ul class="category wawa">
        <li><a href="../Hassan/index.html">Web Designer <br><span class="number">324</span> <span>Open position</span></span><i class="ion-ios-arrow-forward"></i></a></li>
        <li><a href="../Hassan/index.html">Customer Service <br><span class="number">564</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        <li><a href="../Hassan/index.html">Marketing &amp; Sales <br><span class="number">234</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        <li><a href="../Hassan/index.html">Software Development <br><span class="number">425</span> <span>Open position</span><i class="ion-ios-arrow-forward"></i></a></li>
        </ul>
        </div>
        </div>
        </div>
        </section>




        <section class="ftco-section img" style="background-image: url(../media/bg_1.jpg.webp); background-position: top center; height: 500px;">
          <div class="container" style="padding-top: 100px;">
          <div class="row">
          <div class="col-md-6">
          <div class="browse-job p-5">
          <span class="icon-search2 icon"><img src="../media/loupe.png"  alt=""></span>
          
          <span class="subheading">Search Job</span>
          <h2 style="font-weight: 900; color: black;">Browse Job by Specialism</h2>
          <p>Leveraging advanced technology and deep market insight, we streamline the job search and recruitment process, making it easier, faster, and more effective for everyone involved.</p>
          </div>
          </div>
          </div>
          </div>
          </section>

          <footer class="ftco-footer ftco-bg-dark ftco-section">
            <div class="container">
            <div class="row mb-5">
            <div class="col-md">
            <div class="ftco-footer-widget mb-4">
            <h2 class="ftco-heading-2">About</h2>
            <p>At Jobpply, we're revolutionizing the way people connect with their next great opportunity.
               Our mission is simple: to bridge the gap between talented individuals seeking meaningful careers and innovative companies looking for dynamic talent.
              Join us, and let's shape the future of work together.</p>
            <ul class="ftco-footer-social list-unstyled float-md-left float-lft mt-3">
            <li class="ftco-animate"><a style="padding-right: 30px;" href="https://twitter.com" target="_blank"><img style="width: 25px;" src="../media/twitter.png" alt=""></a></li>
            <li class="ftco-animate"><a style="padding-right: 30px;" href="https://facebook.com" target="_blank"><img style="width: 25px;" src="../media/facebook-app-symbol.png" alt=""></a></li>
            <li class="ftco-animate"><a href="https://instagram.com" target="_blank"><img style="width: 25px;" src="../media/instagram.png" alt=""></a></li>
            
            </ul>
            </div>
            </div>
            <div class="col-md">
            <div class="ftco-footer-widget mb-4">
            <h2 class="ftco-heading-2">Employers</h2>
            <ul class="list-unstyled" >
            <ul class="list-unstyled" >
            <li><a href="#" class="py-2 d-block">How it works</a></li>
            <li><a href="#" class="py-2 d-block">Register</a></li>
            <li><a href="#" class="py-2 d-block">Post a Job</a></li>
            <li><a href="#" class="py-2 d-block">Advance Skill Search</a></li>
            <li><a href="#" class="py-2 d-block">Recruiting Service</a></li>
            <li><a href="#" class="py-2 d-block">Blog</a></li>
            <li><a href="#" class="py-2 d-block">Faq</a></li>
            </ul>
            </div>
            </div>
            <div class="col-md">
            <div class="ftco-footer-widget mb-4 ml-md-4">
            <h2 class="ftco-heading-2">Workers</h2>
            <ul class="list-unstyled">
            <li><a href="#" class="py-2 d-block">How it works</a></li>
            <li><a href="#" class="py-2 d-block">Register</a></li>
            <li><a href="#" class="py-2 d-block">Post Your Skills</a></li>
            <li><a href="#" class="py-2 d-block">Job Search</a></li>
            <li><a href="#" class="py-2 d-block">Employer Search</a></li>
            </ul>
            </div>
            </div>
            <div class="col-md">
            <div class="ftco-footer-widget mb-4">
            <h2 class="ftco-heading-2">Have a Questions?</h2>
            <div class="block-23 mb-3">
            <ul>
            <li><span class="icon icon-map-marker"></span><span class="text">103 Faculté des Sciences et Techniques, Casablanca Settat ,Morocco  </span></li>
            <li><a href="#"><span class="icon icon-phone"></span><span class="text">+212 664025605</span></a></li>
            <li><a href="#"><span class="icon icon-envelope"></span><span class="text"><span class="__cf_email__" data-cfemail="b6dfd8d0d9f6cfd9c3c4d2d9dbd7dfd898d5d9db">BoutrasseytWassim @gmail.com</span></span></a></li>
            </ul>
            </div>
            </div>
            </div>
            </div>
            </div>
            </footer>
        

    <script src="main.js"></script>
    <script src="../btsp/js/bootstrap.js"></script>
</body>
</html>